...
def validate_pin_option(config):
    if config["pin_option"] == "single":
        ...
        if "input_pin" not in config or "output_pin" not in config:
            raise cv.Invalid(
                "When using 'split_io', both 'input_pin' and 'output_pin' must be provided"
            )
    return config
...
async def to_code(config):
    ...
    pin_config = config["pin_option"]
    cg.add(var.OneWireBusPinConfig(pin_config))
    if pin_config == "single":
        pin = await cg.gpio_pin_expression(config[CONF_PIN])
        cg.add(var.set_pin(pin))
    elif pin_config == "split_io":
